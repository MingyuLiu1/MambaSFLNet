...
from .vmamba import VSSFreqLocalBlock
...
class MambaLFNet(nn.Module):
    def __init__(self, 
        ...
        super(MambaLFNet, self).__init__()

        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)

        self.encoder_level1 = nn.Sequential(*[VSSFreqLocalBlock(hidden_dim=dim) if i == num_blocks[0] - 1 else VSSFreqLocalBlock(hidden_dim=dim) 
                                              for i in range(num_blocks[0])])
        
        self.down1_2 = Downsample(dim) ## From Level 1 to Level 2
        self.encoder_level2 = nn.Sequential(*[VSSFreqLocalBlock(hidden_dim=int(dim*2**1)) if i == num_blocks[1] - 1 else VSSFreqLocalBlock(hidden_dim=int(dim*2**1))
                                              for i in range(num_blocks[1])])
        
        self.down2_3 = Downsample(int(dim*2**1)) ## From Level 2 to Level 3
        self.encoder_level3 = nn.Sequential(*[VSSFreqLocalBlock(hidden_dim=int(dim*2**2)) if i == num_blocks[2] - 1 else VSSFreqLocalBlock(hidden_dim=int(dim*2**2)) 
                                              for i in range(num_blocks[2])])

        self.up3_2 = Upsample(int(dim*2**2)) ## From Level 3 to Level 2
        self.reduce_chan_level2 = nn.Conv2d(int(dim*2**2), int(dim*2**1), kernel_size=1, bias=False)
        self.decoder_level2 = nn.Sequential(*[VSSFreqLocalBlock(hidden_dim=int(dim*2**1), decoder=True) if i == num_blocks[1] - 1 else VSSFreqLocalBlock(hidden_dim=int(dim*2**1), decoder=True)
                                            for i in range(num_blocks[1])])  
        
        self.up2_1 = Upsample(int(dim*2**1))  ## From Level 2 to Level 1
        self.decoder_level1 = nn.Sequential(*[VSSFreqLocalBlock(hidden_dim=int(dim*2**1), decoder=True) if i == num_blocks[0] - 1 else VSSFreqLocalBlock(hidden_dim=int(dim*2**1), decoder=True)
                                            for i in range(num_blocks[0])])
        
        self.refinement = nn.Sequential(*[VSSFreqLocalBlock(hidden_dim=int(dim*2**1), decoder=True) for i in range(num_refinement_blocks)])
